...
画像生成モデルは、`agy` CLIから利用できるGoogle提供モデルを使う前提です。

## セットアップ

リポジトリを取得したら、利用しているOSに合わせてセットアップスクリプトを実行します。
オプションを指定しなかった場合は、実行後にグローバルへ入れるか、このリポジトリ内だけに入れるかを選択できます。
...
対話なしでインストール先を固定したい場合や、スクリプトから実行する場合:
...
`global`は`$CODEX_HOME/skills`または`~/.codex/skills`に入れます。`local`はこのリポジトリ内の`.codex/skills`に入れます。
...
- `agy` CLIでGoogle提供の画像生成モデルを選択できること
...
Google提供モデルが見つからない場合も、別プロバイダーのモデルへ勝手に切り替えません。

## 生成時の流れ

1. Codexが画像の目的、構図、スタイル、サイズ、枚数、保存先を整理します。
2. `agy --help`などで、その環境に入っている`agy` CLIの実際の使い方を確認します。
3. `agy` CLIで利用できるGoogle提供の画像生成モデルを選びます。
4. `agy` CLIで画像を生成します。
5. 出力ファイルが存在し、空でないことを確認します。
6. 生成された画像のパスを返します。
